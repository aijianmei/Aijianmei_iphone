...
@implementation ShareToQQWeiboController
...
- (void)viewDidLoad
{
    ...
    tencentWeiboManager.tengxunWeibo.accessTokenKey = self.accessTokenKey;
    tencentWeiboManager.tengxunWeibo.accessTokenSecret = self.accessTokenSecret;




    if (self.accessTokenKey == nil || self.accessTokenSecret ==nil) {
        ...
        ///让manager 获得 requestTokenKey 和  requestTokenSecret
        tencentWeiboManager.tengxunWeibo.requestTokenKey = self.requestTokenKey;
        tencentWeiboManager.tengxunWeibo.requestTokenSecret = self.requestTokenSecret;
...
- (void)sendQQWeibo:(id)sender
{
	...
	self.connection	= [api publishMsgWithConsumerKey:self.qqWeiBoAppKey
									  consumerSecret:self.qqWeiBoAppSecret
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    PPDebug(@"what is the query :%@",query);
    PPDebug(@"what is the verifier :%@",verifier);
	
    
	if (verifier && ![verifier isEqualToString:@""]) {
        
		QWeiboSyncApi *api = [[[QWeiboSyncApi alloc] init] autorelease];
		NSString *retString = [api getAccessTokenWithConsumerKey:self.qqWeiBoAppKey
												  consumerSecret:self.qqWeiBoAppSecret
